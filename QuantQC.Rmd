...
```{r negative_ctrl ,echo=FALSE, fig.width = 11, fig.height = 4.5}
...
pep_number <- ggplot(numb_pep, aes(x = number_peptides, fill = filt)) + geom_histogram(bins = 30,position = 'identity',alpha = .5) + ggtitle('# precursors per sample') + rremove('legend')


prot_number<- ggplot(numb_prot, aes(x = number_proteins, fill = filt)) + geom_histogram(bins = 30,position = 'identity',alpha = .5) + ggtitle('# proteins per sample')
...
mp <- ggplot(missingness_prot_mat, aes(x = value, fill = Var2)) + 
  geom_histogram(bins = 20,position = 'identity',alpha = .5) + ggtitle(paste0('Protein completness, ',nrow(protein_mat_NF),' proteins'))+rremove('legend')
